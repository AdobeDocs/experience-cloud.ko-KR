---
title: 지원 받기
description: 셀프서비스 리소스를 포함한 Adobe Experience Rollouts에 대한 지원을 받는 방법 및 지원이 필요할 때 지원 요청을 제출하는 방법에 대해 알아봅니다.
exl-id: f4ba9c00-941b-4c33-b42f-087113d58b0d
source-git-commit: 4a3133f014a9bb9d6ed26eb9d9f763db79ce63b3
workflow-type: tm+mt
source-wordcount: '188'
ht-degree: 1%

---

# 지원 받기 {#get-support}

설명서 또는 내장된 문제 해결 도구 를 사용하여 해결할 수 없는 경험 롤아웃 문제가 발생하는 경우 경험 롤아웃 지원 팀에서 도움을 받을 수 있습니다.

## 셀프서비스 리소스 {#self-service}

지원 요청을 제출하기 전에 다음 셀프서비스 옵션을 사용해 보십시오.

* **이 설명서 검색** - 검색 기능을 사용하여 안내서, 개념 설명 및 방법 콘텐츠를 찾을 수 있습니다.
* **기능 워크벤치 사용** - 코드를 작성하지 않고 특정 사용자에 대한 기능 플래그 평가 문제를 진단합니다. 지침은 [문제 해결](troubleshooting.md)을 참조하세요.
* **기능 플래그 상태 확인** — 기능 플래그가 게시되었고 대상 규칙이 올바르게 구성되었는지 확인합니다.

## 지원 요청 제출 {#submit-request}

셀프서비스 리소스로 문제가 해결되지 않으면 지원 요청을 제출하십시오. 경험 롤아웃 지원 팀에 연결하는 방법에 대한 자세한 내용은 [지원 팀에 문의](contact-support.md)를 참조하세요.

요청을 제출할 때는 가능한 한 많은 세부 정보를 포함하여 팀이 문제를 신속하게 조사하고 해결할 수 있도록 하십시오. [지원에 문의](contact-support.md)하여 정보 확인 목록을 확인하세요.

## 참조 - {#see-also}

* [지원 문의](contact-support.md)
* [문제 해결](troubleshooting.md)
