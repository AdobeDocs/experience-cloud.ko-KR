---
title: iOS 통합을 위한 Experience Rollout Extension 안내서
description: iOS에서 Experience Rollout 확장을 Adobe Experience Platform Mobile SDK과 통합하는 방법을 알아봅니다.
...
# iOS용 경험 롤아웃 확장 {#ios-extension-integration-guide}

이 안내서에서는 iOS에서 Experience Rollout 확장을 Adobe Experience Platform Mobile SDK과 통합하는 방법을 설명합니다.
...
#### Swift 패키지 관리자 사용(권장) {#swift-package-manager}

1. Xcode에서 **파일** > **패키지 종속성 추가**&#x200B;로 이동합니다.
1. Adobe Experience Platform Mobile SDK 저장소 URL을 입력합니다.
...
1. 다음 패키지를 추가합니다.
   ...
   | AEPRolout | `https://github.com/adobe/aepsdk-rollout-ios` |

#### CocoaPod 사용 {#cocoapods}

`Podfile`에 다음 Pod를 추가하십시오.
...
>[!IMPORTANT]
>
>프로덕션 응용 프로그램의 경우 Adobe에서는 `~>` 또는 열린 범위를 사용하는 대신 명시적 버전 번호에 고정하는 것이 좋습니다. 자세한 내용은 [CocoaPods 버전 관리 가이드](https://guides.cocoapods.org/using/the-podfile.html)를 참조하십시오.

### SDK 초기화 {#initialize-sdk}

Experience Rollout 확장 API를 호출하기 전에 `AppDelegate`(또는 `SceneDelegate`)에서 Mobile SDK을 초기화하십시오. 모바일 속성의 환경 파일 ID를 사용하면 앱이 데이터 수집에 게시한 롤아웃 설정을 선택하게 됩니다.
...
>[!IMPORTANT]
>
>프로덕션 앱의 경우 `LogLevel.error`만 사용합니다. 릴리스 빌드에 `.debug` 또는 `.verbose`을(를) 사용하지 마십시오.
...
| `withAttributes(_:)` | 아니요 | `[String: [String]]`. 키는 롤아웃 규칙에 사용되는 컨텍스트 특성 이름입니다(예: `locale`, `platform`, `appVersion`, `deviceType`). 값은 현재 사용자/세션에 대한 해당 키의 후보 특성 값 목록입니다(예: `["en_US"]` 또는 `["phone"]`). |
...
`isFeatureEnabled`은(는) 특정 컨텍스트에 대해 경험 롤아웃 기능이 설정되어 있는지 여부를 반환합니다. `featureKey`, `FeatureEvaluationContext`(Analytics에 대한 선택적 타기팅 특성 및 선택적 ID) 및 완료 처리기를 전달합니다. [평가 컨텍스트](#evaluation-context)를 참조하십시오.
...
| `featureKey` | 문자열 | 경험 롤아웃에서 평가할 기능 키 |
| `evaluationContext` | 기능 평가 컨텍스트 | 필요에 따라 분석에 대한 타기팅 특성 및 선택적 ID를 포함합니다. 빈 컨텍스트의 경우 `FeatureEvaluationContext.builder().build()`을(를) 사용합니다. [평가 컨텍스트](#evaluation-context)를 참조하십시오. |
| `completion` | `(FeatureEvaluationResult?) -> Void` | 평가된 기능 페이로드를 사용하여 호출됩니다. 기능을 찾을 수 없는 경우 `nil`일 수 있습니다. |
...
| API | 반환 |
|---|---|
| `isFeatureEnabled(featureKey:evaluationContext:completion:)`. `FeatureEvaluationContext`은(는) 규칙에 대한 타깃팅 특성과 analytics에 대한 선택적 ID를 전달합니다. [isFeatureEnabled](#is-feature-enabled)을(를) 참조하십시오. | 완료 핸들러를 통한 부울 |
| `getFeature(featureKey:evaluationContext:completion:)`. 특정 컨텍스트에 대해 평가된 기능 페이로드를 반환합니다. [getFeature](#get-feature)을(를) 참조하십시오. | 기능 평가 결과? 완료 처리기 사용 |
| `refreshCache()` | 무효 |
