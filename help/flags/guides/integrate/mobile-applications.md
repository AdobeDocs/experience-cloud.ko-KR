---
title: 모바일 애플리케이션
description: 모바일 SDK을 사용하여 모바일 애플리케이션에 플래그를 통합하는 방법을 알아봅니다.
hide: true
exl-id: 27a43994-25e7-4a2c-b01c-ae98d089413d
source-git-commit: b3e13a42c19eb7b8b7d5d402bf9a56f89f573044
workflow-type: tm+mt
source-wordcount: '65'
ht-degree: 9%

---

# 모바일 애플리케이션 {#mobile-applications}

모바일 애플리케이션은 모바일 SDK을 통해 플래그와 통합됩니다.

>[!NOTE]
>
>모바일 SDK 설명서는 현재 준비 중이며 곧 제공될 예정입니다. 조기 액세스 지침은 Adobe 담당자에게 문의하십시오.

## 통합 단계 {#integration-steps}

통합 프로세스에 대한 자세한 내용은 [통합 단계](integration-steps.md)를 참조하십시오.

## 참조 - {#see-also}

* [통합 단계](integration-steps.md)
* [시작 안내서](startup-guide.md)

<!-- -->
